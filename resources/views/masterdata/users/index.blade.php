            ...
            <div class="py-4 px-2 mx-auto lg:m-8 sm:m-4">
                ...
                <div class="mb-3 flex items-center justify-between">
                    <a href="{{ route('masterdata.users.create') }}" class="inline-block">
                        <button type="button" class="text-white bg-indigo-600 hover:bg-indigo-700 transition duration-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                            Tambah Data
                        </button>
                    </a>
                    ...
                    <form action="{{ route('masterdata.users.index') }}" method="GET" class="flex items-center">
                        ...
                        <button type="submit" class="ml-2 bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 transition duration-300">
                            Search
                        </button>
                        
                        @if (request('search'))
                            <a href="{{ route('masterdata.users.index') }}" class="ml-2 bg-gray-300 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-400 transition duration-300">
                                Reset
                            </a>
                        @endif
                    </form>
                            ...
                            @forelse ($users as $user)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <td class="px-3 py-2 flex space-x-2 justify-center ">
                                        ...
                                        @endif
                                        <form action="{{ route('masterdata.students.destroy', $user->id) }}" method="POST"
                                            ...
                                            <form
                                                action="{{ route('masterdata.users.destroy', $user->id) }}"
